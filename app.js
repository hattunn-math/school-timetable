...
let state={schemaVersion:SCHEMA_VERSION,classes:["1年1組","1年2組"],teachers:[],base:{},daily:{},history:[],classSettings:{}};
let editing=null;
...
function migrate(data){
  ...
  data.schemaVersion=data.schemaVersion||1; data.classes=data.classes||["1年1組"]; data.teachers=data.teachers||[]; data.base=data.base||{}; data.daily=data.daily||data.overrides||{}; data.history=data.history||[]; data.classSettings=data.classSettings||{};
...
function renderBase(){
  ...
  let h="<tr><th>校時</th>"+DAYS.map(d=>`<th>${d}曜日</th>`).join("")+"</tr>";
  periodsForClass(cls).forEach(p=>{h+=`<tr><th>${p}限</th>`;DAYS.forEach(d=>{const l=getBase(cls,d,p);h+=`<td class="slot" data-day="${d}" data-period="${p}">`;h+=l?`<div class="subject">${esc(l.subject)}</div><div class="teacher">${esc(teachersText(l))}</div>${l.room?`<div class="room">${esc(l.room)}</div>`:""}`:`<div class="empty">空き</div>`;h+="</td>"});h+="</tr>"});
...
function conflictEntries(date){
  ...
  state.classes.forEach(c=>DAYS.forEach(d=>periodsForClass(c).forEach(p=>{const l=getDaily(c,date,d,p);if(!l)return;activeTeachers(l).forEach(t=>{const k=`${d}-${p}-${t}`;(teacherMap[k]??=[]).push({c,d,p,t})});if(l.room){const k=`${d}-${p}-${l.room}`;(roomMap[k]??=[]).push({c,d,p,room:l.room})}})));
...
function init(){
  ...
  $("#dailyClass").onchange=renderDaily;$("#baseClass").onchange=()=>{renderBase();updateZeroPeriodButton()};$("#dailyDate").onchange=renderDaily;
  ...
  $("#renameClassBtn").onclick=()=>{const old=$("#baseClass").value,n=prompt("新しいクラス名",old);if(n&&n!==old&&!state.classes.includes(n)){state.base[n]=state.base[old]||{};delete state.base[old];state.classSettings[n]=state.classSettings[old]||{zeroPeriod:false};delete state.classSettings[old];state.classes=state.classes.map(c=>c===old?n:c);Object.keys(state.daily).forEach(k=>{if(k.endsWith("__"+old)){state.daily[k.replace("__"+old,"__"+n)]=state.daily[k];delete state.daily[k]}});save();renderAll()}};
  $("#deleteClassBtn").onclick=()=>{const c=$("#baseClass").value;if(confirm(`${c}を削除しますか？`)){state.classes=state.classes.filter(x=>x!==c);delete state.base[c];delete state.classSettings[c];save();renderAll()}};
